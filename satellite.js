...
(function () {
    ...
    (function buildColormaps() {
        ...
        // Genuine LINEAR grayscale (the old "grayscale" above is really the
        // stepped Dvorak BD curve). Warm = black, cold = white, stretched over a
        // cloud-useful 300→190 K window so the in-cloud gradient fills the ramp.
        IR_COLORMAPS['graylinear'] = (function () {
            var vmin=160.0, vmax=330.0, hot=300.0, cold=190.0, lut=new Uint8Array(256*4);
            lut[0]=0;lut[1]=0;lut[2]=0;lut[3]=0;
            for (var i=1;i<=255;i++){
                var tb=vmin+(i-1)*(vmax-vmin)/254.0;
                var g=Math.max(0,Math.min(255,Math.round(255*(hot-tb)/(hot-cold))));
                var idx=i*4; lut[idx]=g;lut[idx+1]=g;lut[idx+2]=g;lut[idx+3]=255;
            }
            return lut;
        })();
        ...
        // vivid teal→green→yellow ramp (so subtle low-cloud lines spiraling into
        // an LLC pop), and deep cold convection is muted violet-gray so it
        // doesn't steal the eye. See realtime_ir.js for the twin definition.
        ...
        // (less loud), and cold tops pushed to dark indigo so deep convection
        // recedes hard and the boundary layer keeps the eye.
