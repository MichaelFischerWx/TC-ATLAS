...
// near-overhead to a grazing angle) followed by one slow 360° revolution, so the
// user gets the full 3D view hands-free. Returns to the starting tilt/bearing
// when done. Click again (or toggle 3D off) to stop.
window._arch3DOrbit = function () {
    ...
    function step(ts) {
        ...
        if (p >= 1) {
            ...
            _archOrbitRAF = null; if (btn) btn.classList.remove('active'); return;
        }
        var bearing, pitch;
        if (p < INTRO) {
...
function loadData() {
    ...
    // intensity_changes.json + per-basin climatology charts moved to
    // tc_climatology.html (lazy-fetched there on first modal open).
}
    ...
    document.addEventListener('keydown', function (e) {
        ...
        if (e.key === 'Escape') {
            var modals = [
                ...
                // ACE / intensity / RI / seasonal / LMI modals moved to
                // tc_climatology.html. Their Esc handler lives on that page.
            ];
...
function restoreFromHash() {
    ...
    var _restoreInterval = setInterval(function () {
        ...
        // tab=climatology and tab=environment are handled by an inline
        // <head> redirect to tc_climatology.html, so we never reach this
        // path for those values. Left intentionally absent.
